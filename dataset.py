...
def get_seqs(seq_ids, apply_featuregen=True):
    ...
    for path in (pbar := tqdm(paths, file=sys.stdout)):
        ...
        seqs = np.split(df.select(columns).collect().to_numpy()
                        .reshape((-1, 3, len(columns) // 3)).transpose(0, 2, 1), split_idxs)
        for seq in seqs:
...
class NPZDataset(Dataset):
    ...
    def create(seq_ids, save_path, crop_labels):
        ...
        for i, (x, y) in (pbar := tqdm(enumerate(list(zip(seqs, labels))), file=sys.stdout)):
