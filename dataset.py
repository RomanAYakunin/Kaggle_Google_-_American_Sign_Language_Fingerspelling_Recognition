...
class FeatureGenerator:  # TODO rename
    def __init__(self):
        self.left_points = np.arange(468, 489)
        self.right_points = np.arange(522, 543)  # most prominent signing hand
        self.left_range = (0, 21)
        self.right_range = (21, 42)
        self.all_points = np.concatenate([self.left_points, self.right_points])
        self.num_points = len(self.left_points)
        self.num_axes = 3
...
def get_column_names(filter_columns=True):
    ...
    if filter_columns:
        FG = FeatureGenerator()
        columns = columns.reshape(3, -1)[:FG.num_axes, FG.all_points].flatten()
    return columns


def get_seqs(seq_ids, filter_features=True, float16=False):
    FG = FeatureGenerator()
    num_axes = FG.num_axes if filter_features else 3
    columns = get_column_names(filter_features)
    ...
    for path in (pbar := tqdm(paths, file=sys.stdout)):
        ...
        seqs = df.select(columns).collect().to_numpy().reshape((-1, num_axes, len(columns) // num_axes))\
            .transpose(0, 2, 1).astype(np.float16 if float16 else np.float32)
        seqs = np.where(np.isnan(seqs), np.zeros_like(seqs), seqs)  # zero out nan
        seqs = np.split(seqs, split_idxs)
        for seq in seqs:
            if filter_features:
                left = seq[:, FG.left_range[0]: FG.left_range[1]]
                right = seq[:, FG.right_range[0]: FG.right_range[1]]
                left_not_nan = np.sum(np.any(left != 0, axis=(1, 2)).astype(int))
                right_not_nan = np.sum(np.any(right != 0, axis=(1, 2)).astype(int))
                if right_not_nan >= left_not_nan:  # choose dominant hand
                    seq = right
                else:
                    seq = left
                    seq[..., 0] *= -1  # reflect x coords if left
                seq = seq[np.any(seq != 0, axis=(1, 2))]  # remove nan frames
            file_seq_list.append(seq[:FG.max_len])  # append cropped seq
        if len(split_idxs) == 0:
