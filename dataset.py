...
class FeatureGenerator(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x):  # TODO adjust based on num_axes
        x = torch.index_select(x[:self.max_len], dim=1, index=self.point_arr)  # [num_frames, num_points, num_axes]
        x = torch.where(torch.isnan(x), torch.full_like(x, fill_value=self.nan_value), x)
        return x


def get_column_names(apply_featuregen=True):
...
def get_seqs(seq_ids, apply_featuregen=True):
    FG = FeatureGenerator()
    columns = get_column_names(apply_featuregen)
    ...
    for path in (pbar := tqdm(paths, file=sys.stdout)):
        ...
        seqs = df.select(columns).collect().to_numpy().reshape((-1, 3, len(columns) // 3)).transpose(0, 2, 1)\
            .astype(np.float16)
        seqs = np.where(np.isnan(seqs), np.full_like(seqs, fill_value=FG.nan_value), seqs)
...
def get_dataloader(save_path, batch_size, shuffle):
    ...
    def collate_fn(batch):
        ...
        for i in range(len(chunks)):
            ...
            if len(x) == 0:
                continue
            x = pad_sequence(x, batch_first=True, padding_value=FG.nan_value)
            y = torch.cat(y)
